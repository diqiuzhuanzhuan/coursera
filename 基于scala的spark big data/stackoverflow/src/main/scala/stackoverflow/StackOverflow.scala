...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
	  ...
	  def answerHighScore(as: Array[Posting]): Int = {
		...
		/*
	  grouped.flatMap(x => x._2).groupByKey().map(x => {
		  (x._1, answerHighScore(x._2.toArray))
	  })
	  */
  }
...
//    val newMeans = means.clone() // you need to compute newMeans
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
	  ...
	  val median = closestGrouped.mapValues { vs =>
			...
			val res = vs.map(_._1 / langSpread).groupBy(identity(_)).mapValues(_.size)
			val index = res.maxBy(_._2)._1
			val langLabel: String   = langs(res.maxBy(_._2)._1)
		  // most common language in the cluster
		  val langPercent: Double = res(index) * 100.0d / vs.size.toDouble // percent of the questions in the most common language
		  val clusterSize: Int    = vs.size
			val l = vs.map(_._2).toList.sorted
			val mediaIndex = l.size/2
		  val medianScore: Int    = if (vs.size%2 == 0) (l(mediaIndex-1) + l(mediaIndex))/2 else l(mediaIndex)
